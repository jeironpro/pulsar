...
(() => {
  ...
  const makeStars = (canvas, count = 150) => {
    ...
    const resize = () => {
      ...
      stars = Array.from({ length: Math.max(40, n) }, () => ({
        ...
        base: Math.random() * .55 + .25,
        phase: Math.random() * Math.PI * 2,
        speed: Math.random() * .9 + .25,
        color: Math.random() < .1 ? '245,176,74' : (Math.random() < .28 ? '160,150,255' : '238,240,255')
      }));
    };
    ...
    const draw = t => {
      ...
      for (const s of stars) {
        const tw = REDUCED ? 1 : (.5 + .5 * Math.sin(s.phase + t * .001 * s.speed));
    ...
    resize();
    if (REDUCED) { draw(0); cancelAnimationFrame(raf); }
    else {
    ...
    const runTyping = async () => {
      ...
      while (true) {
        for (const step of SCRIPT) {
          ...
          const proxy = { n: 0 };
          await anime({
            targets: proxy, n: fullCmd.length, duration: fullCmd.length * 34,
            easing: 'linear',
            update: () => {
              const shown = fullCmd.slice(0, Math.round(proxy.n));
              $$('.t-cmd,.t-flag,.t-prompt', termBody).forEach(n => n.remove());
              let idx = 0;
          ...
          for (const [c, t] of step.out) {
            ...
            termBody.insertBefore(document.createTextNode('\n'), cursor);
            anime({ targets: o, opacity: [0, 1], duration: 220, easing: 'easeOutQuad' });
            await sleep(90);
          }
          await sleep(1500);
        }
        [...termBody.childNodes].forEach(n => { if (n !== cursor) n.remove(); });
        await sleep(400);
      }
    };
  ...
  if (tooltipEl) {
    ...
    const hideTip = () => {
      ...
      if (cleanupAuto) { cleanupAuto(); cleanupAuto = null; }
    };
    ...
    $$('[data-tip],[data-tip-html]').forEach(el => {
      ...
      el.addEventListener('mouseenter', () => { el.__tipContent = getContent(); showTip(el, el.__tipContent); });
      el.addEventListener('focusin', () => { el.__tipContent = getContent(); showTip(el, el.__tipContent); });
  ...
  $$('.copy-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const text = btn.dataset.copyText
        ?? ($(btn.dataset.copyTarget)?.textContent ?? '');
      if (!text) return;
  ...
  tabs.forEach((tab, i) => {
    ...
    tab.addEventListener('keydown', e => {
      ...
      if (j !== null) { e.preventDefault(); tabs[j].focus(); activateTab(tabs[j]); }
    });
  });
